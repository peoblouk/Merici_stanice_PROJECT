...
uint32_t loop_count; // helping value
...
{
	delay_reset();
	for (loop_count = 0; loop_count < time_ms; loop_count++)
	{
		printf("nacitani registru: %d\n", loop_count);
...
{
	delay_reset();
	for (loop_count = 0; loop_count < (time_us / 1000); loop_count++)
	{
		while (TIM4_GetFlagStatus(TIM4_FLAG_UPDATE) != SET)
			;
		TIM4_ClearFlag(TIM4_FLAG_UPDATE);
